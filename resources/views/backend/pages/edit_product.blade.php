      ...
      <div class="container-fluid">
         ...
         <form action="{{ url('admin/update_product') }}" method="POST" enctype="multipart/form-data">
               ...
               <div class="col-sm-10">
                  <select class="form-control" name="category_id">
                     ...
                     @foreach($category as $row)
                        <option @if($row->id == $product->cat_id )  selected="selected"  @endif  value="{{ $row->id }}">{{ $row->category_name }}</option>
                     @endforeach
                  </select>
               </div>
            </div>
            ...
            <div class="form-group row">
               ...
               <div class="col-sm-10">
                  <input type="text" class="form-control" id="desc" value="{{ $product->description }}" name="description" placeholder="Description">
               </div>
            </div>
            <div class="form-group row">
